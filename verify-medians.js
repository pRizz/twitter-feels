// Sam Altman - 29 tweets emotion scores (from verify-aggregations.js)
const samTweets = [
...
// API returned medians for Sam Altman
const apiMedians = {"happy":45,"sad":25,"angry":21,"fearful":16,"hatred":9,"thankful":35,"excited":54,"hopeful":30,"frustrated":26,"sarcastic":17,"inspirational":38,"anxious":17};

const manualMedians = calculateMedians(samTweets);

console.log('=== Sam Altman Median Verification (29 tweets) ===\n');
...
emotions.forEach(emotion => {
  ...
  const match = manual === api;
  if (!match) allMatch = false;
  console.log(`${emotion}: manual=${manual}, api=${api} ${match ? '✓ MATCH' : '✗ MISMATCH'}`);
});

console.log('\n========================================');
console.log('FINAL RESULT:', allMatch ? '✓ ALL MEDIANS MATCH' : '✗ SOME MEDIANS DO NOT MATCH');
console.log('========================================');

// Show detailed calculation for 'happy' emotion as verification
console.log('\n=== Detailed Happy Median Calculation ===');
const happyValues = samTweets.map(t => t.happy);
console.log('Raw happy values:', happyValues.join(', '));
const sortedHappy = [...happyValues].sort((a, b) => a - b);
console.log('Sorted:', sortedHappy.join(', '));
console.log('Length:', sortedHappy.length, '(odd, so median is middle value)');
console.log('Middle index:', Math.floor(sortedHappy.length / 2), '= index 14');
console.log('Median value:', sortedHappy[14]);
